...
-- Total Number of companies to go under by country
WITH Ended_Companies AS (
SELECT * FROM layoffs_staging2
WHERE percentage_laid_off = 1
ORDER BY total_laid_off DESC
)
SELECT country, COUNT(country) FROM Ended_Companies
GROUP BY country
ORDER BY 2 DESC;
...
-- Total Number of companies by industry to go under
WITH Ended_Companies AS (
SELECT * FROM layoffs_staging2
WHERE percentage_laid_off = 1
ORDER BY total_laid_off DESC
)
SELECT industry, COUNT(industry) FROM Ended_Companies
GROUP BY industry
ORDER BY 2 DESC;
...
-- Average percentage of layoffs by country
SELECT RANK() OVER(ORDER BY AVG(percentage_laid_off) DESC) as 'Rank', country, AVG(percentage_laid_off), COUNT(country) AS num_company_layoffs FROM layoffs_staging2
GROUP BY country
ORDER BY 'Rank' DESC;

-- Average percentage of layoffs by country with minimum 10 company layoff periods across all companies
SELECT country, AVG(percentage_laid_off), COUNT(country) AS num_company_layoffs FROM layoffs_staging2
GROUP BY country
